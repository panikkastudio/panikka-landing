...
const transformClass =
  "transform transition duration-500 hover:scale-[1.02] hover:drop-shadow-xl ";

export function Magicshorts() {
  return (
    <div
      className={`group relative flex flex-col h-[280px] col-span-2 bg-primary rounded-3xl hover:cursor-alias`}
    >
      ...
      <div className="flex flex-col justify-between items-start max-w-[350px] h-full">
        <div className="text-black py-3">
          ...
          <p className="font-bold text-lg">
            Interview Candidates on Auto-Pilot
          </p>
        </div>
        <p>
          Capture authentic video responses, enhance candidate evaluation, and
          simplify your hiring process — all in one place.
        </p>
        <Image
          src={vuescout}
          alt="Vuescout - Interview Candidates on Auto-Pilot"
          className="w-auto h-[72px] pt-1 "
        />
      </div>
...
export function Vizco() {
  return (
    <div
      className={`relative group flex flex-col h-[280px] col-span-2 bg-[#CAEFFD] rounded-3xl hover:cursor-alias`}
    >
      ...
      <div className="relative h-full">
        <Image
          src={vizco}
          alt="Vizco - Online Audiogram Maker"
          className="absolute h-full w-full rounded-b-3xl object-cover object-top"
        />
...
export function AppImage({
  ...
  return (
    <div
      className={`${className} group relative sm:flex rounded-3xl w-full h-32 `}
    >
      <Image
        src={src}
        alt={alt}
        className={`w-full h-full object-cover rounded-3xl hover:cursor-alias`}
      />
